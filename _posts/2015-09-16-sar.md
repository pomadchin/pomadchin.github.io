...
* Also possible another _SAR platforms_, which listed nether here nor in the official documentation

This briefly analysis shows that ASF API provides very old, and hardly refreshable data. The description for some products was not found, but known, that this data is old and can be used for historical research only.
...
What is in the end? Why only ASF API observed? ASF API is a sort of SAR data sources aggregation. The only actual data (free and in public sources, found during this small research) is provided by [ALOS-2](http://www.jaxa.jp/projects/sat/alos2/index_j.html) satellite, but, there are no sources (or were not found) providing the daily whole earth coverage. In conclusion, it is possible to add, that the SAR data is one of the hardest things to find, may be there are any other sources of commercial data, but it is a topic for antother post.